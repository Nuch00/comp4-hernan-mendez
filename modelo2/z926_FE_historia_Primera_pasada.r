...
meses= c( 201903, 201903, 201904, 201905, 
          201906, 201907, 201908, 201909, 
          201910, 201911, 201912, 202001, 
          202002, 202011, 202012, 202101, 
...
dvalid  <- lgb.Dataset( data=    data.matrix(  dataset[ foto_mes==mes_valid, campos_buenos, with=FALSE]),
                        label=   dataset[ foto_mes==mes_valid, clase01],
                        weight=  dataset[ foto_mes==mes_valid, ifelse(clase_ternaria=="BAJA+2", 1.0000001, 1.0)],
                        free_raw_data= FALSE
)
...
ref = which(tb_importancia$Gain>0.01)
if(length(ref)>10) ref = seq(1,10)


variables_buenas = c(tb_importancia$Feature[ref],'cliente_edad','cliente_antiguedad')
# 
